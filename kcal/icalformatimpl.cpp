                                                  ...
                                                  IncidenceBase *incidenceBase )
{
  ...
  if ( !incidenceBase->organizer().isEmpty() ) {
    icalcomponent_add_property( parent, mImpl->writeOrganizer( incidenceBase->organizer() ) );
  }
...
icalproperty *ICalFormatImpl::writeAttendee(Attendee *attendee)
{
  ...
  if ( !attendee->delegate().isEmpty() ) {
    icalparameter* icalparameter_delegate = icalparameter_new_delegatedto( attendee->delegate().toUtf8() );
    icalproperty_add_parameter( p, icalparameter_delegate );
  }

  if ( !attendee->delegator().isEmpty() ) {
    icalparameter* icalparameter_delegator = icalparameter_new_delegatedfrom( attendee->delegator().toUtf8() );
...
//                  << endl;
  ...
  if (d->mCompat) d->mCompat->fixEmptySummary( todo );
    ...
    switch (kind) {
      ...
      case ICAL_DTEND_PROPERTY: { // end date and time
        ...
        if ( kdt.isDateOnly() ) {
          ...
          if ( d->mCompat ) d->mCompat->fixFloatingEnd( endDate );
...
//    kDebug(5800) << "ICALFormat::readEvent(): all day event: " << floats << endl;
  ...
  if ( d->mCompat ) d->mCompat->fixEmptySummary( event );
...
//                  << endl;
  ...
  if ( d->mCompat ) d->mCompat->fixAlarms( incidence );
}
...
//    kDebug(5800) << "VCALENDAR prodid: '" << d->mLoadedProductId << "'" << endl;
...
//    kDebug(5800) << "VCALENDAR version: '" << version << "'" << endl;
    ...
    if (strcmp(version,"1.0") == 0) {
      ...
      d->mParent->setException(new ErrorFormat(ErrorFormat::CalVersion1,
