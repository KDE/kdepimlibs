...
void Content::parse()
{
  ...
  if( ct->isText() ) {
    ...
    kDebug() << "Content-Type is" << ct->mimeType() << "; leaving untouched.";
  }
}
...
QByteArray Content::assembleHeaders()
{
  ...
  foreach( const Headers::Base *h, h_eaders ) {
    kDebug() << "header" << h->type() << h->as7BitString();
    if( !h->isEmpty() ) {
...
void Content::addContent( Content *c, bool prepend )
{
  ...
  if( d->contents.isEmpty() && !contentType()->isMultipart() ) {
    ...
    for ( Headers::Base::List::iterator it = h_eaders.begin();
          it != h_eaders.end(); ) {
      if ( (*it)->isMimeHeader() ) {
        kDebug() << "moving header" << (*it)->type() << (*it)->as7BitString();
        // Add to new content.
        main->setHeader( *it );
        // Remove from this content.
        it = h_eaders.erase( it );
      } else {
        kDebug() << "skipping header" << (*it)->type() << (*it)->as7BitString();
        ++it;
      }
    }
    ...
    {
      main->assemble(); // For debugging only.
      kDebug() << "head of new subcontent" << main->head();
    }
...
void Content::removeContent( Content *c, bool del )
{
  ...
  if( d->contents.count() == 1 ) {
    ...
    foreach( Headers::Base *h, main->h_eaders ) {
      kDebug() << "moving header" << h->type() << h->as7BitString();
