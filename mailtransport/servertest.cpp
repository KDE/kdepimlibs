...
KIO::MetaData ServerTest::slaveConfig() const {
  KIO::MetaData md;
  md.insert( QLatin1String("nologin"), QLatin1String("on") );
  return md;
}

void ServerTest::startOffSlave( int port ) {
  KUrl url;
  url.setProtocol( mSSL ? mProtocol + QLatin1Char('s') : mProtocol );
  url.setHost( mHost );
  if ( port )
    url.setPort( port );
...
{
  if ( mSSL )
    mListSSL = data.split(QLatin1Char(' '), QString::SkipEmptyParts);
  else
    mListNormal = data.split(QLatin1Char(' '), QString::SkipEmptyParts);
}


void ServerTest::slotMetaData( const KIO::MetaData & md ) {
  KIO::MetaData::const_iterator it = md.find( QLatin1String("PLAIN AUTH METHODS") );
  if ( it != md.end() ) {
    mAuthNone = it.value();
    kDebug(5006) << "mAuthNone: " << mAuthNone << endl;
  }
  it = md.find( QLatin1String("TLS AUTH METHODS") );
  if ( it != md.end() ) {
    mAuthTLS = it.value();
    kDebug(5006) << "mAuthTLS: " << mAuthTLS << endl;
  }
  it = md.find( QLatin1String("SSL AUTH METHODS") );
  if ( it != md.end() ) {
    mAuthSSL = it.value();
  ...
  const QString &errorText)
{
  ...
  if ( mSSL && error == 0 ) {
    ...
    mListSSL.append(QLatin1String("SSL"));
  }
  ...
  if ( error )
  {
    ...
    if ( error == 1 )
    {
      ...
      errorMessage = i18n( "Starting the ioslave for protocol %1 failed.",
                           mSSL ? mProtocol + QLatin1Char('s') : mProtocol );
    }
    else
    {
  ...
  if (!mSSL) {
    mSSL = true;
    mListNormal.append(QLatin1String("NORMAL-CONNECTION"));
    startOffSlave();
  } else {
    mJob = 0;
